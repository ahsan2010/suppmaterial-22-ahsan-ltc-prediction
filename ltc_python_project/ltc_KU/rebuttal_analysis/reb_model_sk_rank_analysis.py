...
reb_model = [KULTC_SUM_AUTO,XIN_FEAT]
reb_model_II = [KULTC_SUM_AUTO,XIN_FEAT,KULTC_SUM_XIN_AUTO]
...
def generate_sk_data_rebuttal(model_list, auto_string, tag):
    ltc_year_list = [1, 2, 3]
    for year in ltc_year_list:
        model_sk_data = {}
        for model_name in model_list:
        ...
        #sk_model_input_year_rq6-f2_3
        sk_output_file = "{}sk_model_input_year_{}_{}.txt".format(sk_analysis_dir, tag, year)
        with open(sk_output_file, 'w') as fp:
            for m in model_list:
                auc_value_str = ' '.join([str(x) for x in model_sk_data[m]])
                fp.write(m+auto_string)
...
def rebuttal_rank_data():
    generate_sk_data_rebuttal(reb_model, "", "reb-rq1")
    generate_sk_data_rebuttal(reb_model_II, "", "reb-rq2")
