...
def windows_based_testing_xin_data(year):
    seed_value = 123
    nfold = 10
    result = []
    for i in range (1,10):
        num_training_window = i
        ltc_data = "/home/local/SAIL/ahsan/BACKUP/ahsan_project_2022/xin_xia_paper_data/data/data_year_{}.csv".format(year)
        pd_data = pd.read_csv(ltc_data)
        num_records = pd_data.shape[0]
        equal_window_size = int(num_records/nfold)
        data_windows = create_equal_size_winow(nfold, num_records, equal_window_size)
        train_index, test_index = get_train_test_split_train_window(num_training_window, data_windows, nfold)
    
        X_train, Y_train, X_test, Y_test = get_test_train_xin_data(pd_data, train_index, test_index, year)
        X = X_train
        y = Y_train
        #oversample =  imblearn.over_sampling.SMOTE(random_state=seed_value)
        #X, y = oversample.fit_resample(X_train, Y_train)
    
        rf_clf = RandomForestClassifier(random_state=seed_value, n_estimators = 100).fit(X, y)
        rf_preds = rf_clf.predict(X_test)
        predicted_proba = rf_clf.predict_proba(X_test)[:,1]
        Y_test.columns = ['target']
        Y_train.columns = ['target']
...
def k_fold_stratified_windows_based_testing(year_value):
    ...
    result_path = '/home/local/SAIL/ahsan/BACKUP/ahsan_project_2022/FeasibilityStudy/results/'
    result_pd.to_csv("{}k_fold_LTC_{}_years.csv".format(result_path, year_value), index = False) 
...
def k_fold_stratified_windows_based_testing_xin_data(year):
    ...
    for train_index, test_index in skf.split(x, y):
        ...
        predicted_proba = rf_clf.predict_proba(X_test)[:,1]
        #rf_preds = (predicted_proba [:,1] < 0.5).astype('int')
...
def predict_long_time_contributor_xin_data():
    year = 1
    seed_value = 123
    first_year_ltc = "/home/local/SAIL/ahsan/BACKUP/ahsan_project_2022/xin_xia_paper_data/data/data_year_{}.csv".format(year)
    pd_data = pd.read_csv(first_year_ltc)
    ...
    roc_value = roc_auc_score(list(y_test['is_ltc']), y_pred)

def xin_original_paper_replication():
    seed_value = 123
    nfold = 10
    num_training_window = 6
    
    first_year_ltc = "/home/local/SAIL/ahsan/BACKUP/ahsan_project_2022/xin_xia_paper_data/data/data_year_1.csv"
    pd_first_year_ltc = pd.read_csv(first_year_ltc)
...
def test_random_forest():
    ...
    precision, recall, fscore, support, far, tp, fp, tn, fn, roc_value = calculate_eval_metrics_II(y_test,y_pred)
...
if __name__ == "__main__":
    ...
    # windows_based_testing_xin_data(2)
    # windows_based_testing_xin_data(3)
    # k_fold_stratified_windows_based_testing_xin_data(2)
    # k_fold_stratified_windows_based_testing_xin_data(3)
    k_fold_stratified_windows_based_testing_xin_data(1)
