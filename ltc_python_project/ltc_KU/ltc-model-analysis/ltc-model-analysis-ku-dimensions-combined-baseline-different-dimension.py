...
# -*- coding: utf-8 -*-
"""
Created on Sat Feb 17 13:00:51 2024
...
KU_CUR_DEV_EXP = 'ku_cur_dev_exp_dim'
...
rebut_1_model = [KU_CUR_DEV_EXP]

model_ku_xin_comb = [KU_XIN_DEV,
                     KU_XIN_REPO,
                     KU_XIN_DEV_ACT,
                     KU_XIN_REPO_ACT,
                     KU_XIN_COLLAB_NET,
...
def out_of_bootstrap_running(target_name, feature_name, X, Y,  boot_lim, year_value, file_name):
    result = []
...
def bootstrap_model_run(year_value,target_name):
    ...
    feature_list = {
        KU_XIN_DEV : ku_xin_dev,
        KU_XIN_REPO : ku_xin_repo,
        KU_XIN_DEV_ACT : ku_xin_dev_month_act,
        KU_XIN_REPO_ACT: ku_xin_repo_month_act,
        KU_XIN_COLLAB_NET: ku_xin_collab,
        KU_XIN_ALL: ku_xin_all,
        KU_CUR_DEV_EXP : ku_cur_dev_exp
        }
    
    
    for model_name in rebut_1_model:
    #for model_name in model_ku_xin_comb:
    #for i in range(len(model_ku_xin_comb)):
        print(model_name)
        feat = feature_list[model_name]
        X = pd_full_merged [feat]
        
        #result_ku = out_of_bootstrap_running('Ahsan_Java_Commit', model_name, X, Y, 100, year_value = year_value, file_name = model_name)
        #result_ku.to_csv('{}ltc_{}_{}_100.csv'.format(output_path, year_value, model_name))
        #print("Finish: ALL: {} {}".format(year_value, model_name))

        auto_feature = AutoSpearman(X_train = X, verbose=False)
        X_auto = X[auto_feature]
        result_ku_auto = out_of_bootstrap_running('Ahsan_Java_Commit', model_name +"_auto", X_auto, Y, 100, year_value = year_value, file_name = model_name +"_auto")
        result_ku_auto.to_csv('{}ltc_{}_{}_auto_100.csv'.format(output_path, year_value, model_name))
        print("Finish: AUTO {} {}".format(year_value, model_name))
