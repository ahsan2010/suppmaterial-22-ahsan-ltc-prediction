...
# -*- coding: utf-8 -*-
"""
Created on Tue Mar  5 10:37:26 2024
...
def outlier_removal(X):
    row_bool = [True for i in range(X.shape[0])]
    for col in X.columns:
        ...
        if (((X[col].dtype)=='float64') | ((X[col].dtype)=='int64')):
            ...
            lower = Q1 - 1.5*IQR
            upper = Q3 + 1.5*IQR
            rr = (X[col] > lower) & (X[col] < upper)
            row_bool = row_bool & rr
            
    return row_bool

def shap_summary_analysis(load_shap_value, boot_id, X, Y, year_value, file_name):
    ...
    figure_file_name = "{}{}_{}_{}.pdf".format(fig_path,file_name, year_value,boot_id)
    shap.summary_plot(load_shap_value, X_test, feature_names = feature_name_list, plot_size =(8,10),show=False)
...
def main():
    ...
    shap_value_output_dir_combine = "/home/local/SAIL/ahsan/BACKUP/ahsan_project_2022/XinProjectResult/Sep-2023/features/feature-importance-analysis/re-run-shap-values-march-7-2024/xin-ku-all-year/"
    shap_value_output_dir_ku = "/home/local/SAIL/ahsan/BACKUP/ahsan_project_2022/XinProjectResult/Sep-2023/features/feature-importance-analysis/re-run-shap-values-march-7-2024/ku-all-dim-all-year/"
