...
# -*- coding: utf-8 -*-
"""
Created on Thu Feb 22 00:36:19 2024
feature importance on train/test set data for shap
...
def child_thread_shap_analysis(boot_id, file_name, year_value, X, Y, features, output_dir):
    ...
    explainer = shap.Explainer(loaded_model.predict, X, seed = 1)
    shap_values = explainer(X)
    shap_values.feature_names = features
        
    shap_output_file = "{}{}_shap_{}_{}.pkl".format(output_dir,file_name,year_value,boot_id)
...
def multi_run_wrapper(args):
    ...
    features = args[5]
    output_dir = args[6]
    child_thread_shap_analysis(boot_id, file_name, year_value, X, Y, features, output_dir)
    
def parallel_shap_feature_importance(boot_lim, year_value, file_name, output_dir):
    ...
    with get_context("spawn").Pool(10) as p:
        p.map(multi_run_wrapper,[[boot_id,file_name,year_value, X, Y, features, output_dir] for boot_id in sample_iterations])
        p.close()
        p.join()
...
def run_ku_all_auto():
    parallel_shap_feature_importance(100, 1, KU_ALL_AUTO, shap_value_output_dir_ku_all)
    parallel_shap_feature_importance(100, 2, KU_ALL_AUTO, shap_value_output_dir_ku_all)
    parallel_shap_feature_importance(100, 3, KU_ALL_AUTO, shap_value_output_dir_ku_all)
    
def run_xin_ku_all_auto():
    parallel_shap_feature_importance(100, 1, KU_ALL_XIN_ALL_AUTO, shap_value_output_dir_ku_xin_all)
    parallel_shap_feature_importance(100, 2, KU_ALL_XIN_ALL_AUTO, shap_value_output_dir_ku_xin_all)
    parallel_shap_feature_importance(100, 3, KU_ALL_XIN_ALL_AUTO, shap_value_output_dir_ku_xin_all)
