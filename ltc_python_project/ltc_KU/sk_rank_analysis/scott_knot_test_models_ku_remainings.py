...
# -*- coding: utf-8 -*-
"""
Created on Fri Feb 16 12:00:09 2024
...
def analyze_ltc_feature(location):
    random.seed(1)
    project_result = []
    ...
    for r in range(total_elements - 1, -1 , -1):
        ...
        prev = result[r].rank 
    df = pd.DataFrame.from_records(project_result)
    df.columns = ["model_name", "rank","sk_rank"]
    return df

def analyze_sk_rank_start_with_two_models(year_list, model_type_list):
    result_list = []
    for year in year_list:
        print(year)
        for model_type in model_type_list:
            location = "{}sk_model_input_year_{}{}_two.txt".format(sk_analysis_dir,year, model_type)
            df = analyze_ltc_feature(location)
            df['ltc'] = year
            df['model_type'] = 'no-auto' if model_type=='' else 'auto'
            result_list.append(df)
    pd_final = pd.concat(result_list)
    pd_final.to_csv('{}sk_rank_results_two.csv'.format(sk_rank_result_dir),index = False)


def analyze_sk_rank_start_with_different_models(year_list, model_type_list):
    result_list = []
    for year in year_list:
        print(year)
        for model_type in model_type_list:
            location = "{}sk_model_input_year_{}{}_all.txt".format(sk_analysis_dir,year, model_type)
            df = analyze_ltc_feature(location)
            df['ltc'] = year
            df['model_type'] = 'no-auto' if model_type=='' else 'auto'
            result_list.append(df)
    pd_final = pd.concat(result_list)
    pd_final.to_csv('{}sk_rank_results_all.csv'.format(sk_rank_result_dir),index = False)

def analyze_sk_rank_remaining_ku_models(year_list, model_type_list):
    result_list = []
    for year in year_list:
        print(year)
        for model_type in model_type_list:
            location = "{}sk_model_input_year_{}{}_remaining_kus.txt".format(sk_analysis_dir,year, model_type)
            df = analyze_ltc_feature(location)
            df['ltc'] = year
            df['model_type'] = 'no-auto' if model_type=='' else 'auto'
            result_list.append(df)
    pd_final = pd.concat(result_list)
    pd_final.to_csv('{}sk_rank_results_remain_ku_models.csv'.format(sk_rank_result_dir),index = False)


sk_rank_result_dir = "/home/local/SAIL/ahsan/BACKUP/ahsan_project_2022/XinProjectResult/Sep-2023/features/model-sk-analysis/rank-result/"
...
def main():
    year_list = [1, 2, 3]
    model_type_list = ['']
    analyze_sk_rank_remaining_ku_models(year_list , model_type_list)
