...
def generate_sk_rank(location,eval_metric):
    ...
    for r in range(total_elements - 1, -1 ,-1):
        if result[r].rank != prev:
            rank = rank + 1
        row_list = [eval_metric,result[r].rx,rank,result[r].rank]
        project_result.append(row_list)
        prev = result[r].rank 
    df = pd.DataFrame.from_records(project_result)
    df.columns = ["eval_metric","model", "rank","sk_rank"]
    return df


def analyze_ltc_feature(file_name, year_value):
    dir = "/home/local/SAIL/ahsan/BACKUP/ahsan_project_2022/XinProjectResult/Sep-2023/features/feature-importance-analysis/shap-values/sk-input-data/"
    location = "{}{}_sk_input_year_{}.txt".format(dir,file_name,year_value)
    sk_result_dir = "/home/local/SAIL/ahsan/BACKUP/ahsan_project_2022/XinProjectResult/Sep-2023/features/feature-importance-analysis/shap-values/sk-result/"
    random.seed(1)
    project_result = []
    ...
    for r in range(total_elements):
        ...
        prev = result[r].rank 
    df = pd.DataFrame.from_records(project_result)
    df.columns = ["feature", "rank","sk_rank"]
    df.to_csv('{}sk_result_{}_{}.csv'.format(sk_result_dir,file_name, year_value),index = False)

    
def analyze_sk_rank_start_with_different_models():
    sk_analysis_path = "/home/local/SAIL/ahsan/BACKUP/ahsan_project_2021/ahsan_gustavo/dev_knowledge/Result_June_26_2022/Review_Recommendation_Restult/scot_knot_test/"
    model_list = ['CF','RF','ER','KUREC']
    eval_metric_list = ['Accuracy','MAP']
    result_list = []
    for model_name in  model_list:
        print(model_name)
        for eval_metric in eval_metric_list:
            location = '{}Recom_Data_{}_{}.csv'.format(sk_analysis_path, eval_metric, model_name)
            df = generate_sk_rank(location, eval_metric)
            result_list.append(df)
        pd_final = pd.concat(result_list)
        pd_final.to_csv('{}sk_rank_result_start_{}.csv'.format(sk_analysis_path,model_name),index = False)

def anlyze_sk_rank():
    sk_analysis_path = "/home/local/SAIL/ahsan/BACKUP/ahsan_project_2021/ahsan_gustavo/dev_knowledge/Major-EMSE/RecommenderResults/Testing/SK-RANK/"
    #sk_analysis_path = "/home/local/SAIL/ahsan/BACKUP/ahsan_project_2021/ahsan_gustavo/dev_knowledge/Result_June_26_2022/Review_Recommendation_Restult/scot_knot_test/"
    model_list = ["KUREC","CF","RF","CHREV","ER", "CORRECT", "SOFIA", "CN"]
    #model_list = ['CF','RF','ER','CHREV','KUREC','COMGEOM','COMSUM']
    eval_metric_list = ['Accuracy','MAP']
    result_list = []
    for eval_metric in eval_metric_list:
        #location = '{}Recom_Data_{}.csv'.format(sk_analysis_path, eval_metric)
        location = '{}{}_sk_data.csv'.format(sk_analysis_path, eval_metric)
        df = generate_sk_rank(location, eval_metric)
        result_list.append(df)
    
    pd_final = pd.concat(result_list)
    #pd_final.to_csv('{}sk_rank_result.csv'.format(sk_analysis_path),index = False)
    pd_final.to_csv('{}sk_rank_recommenders.csv'.format(sk_analysis_path),index = False)
...
def ltc_feature_sk_ranking():
    ...
    for year_value in year_list:
        analyze_ltc_feature(KU_ALL_AUTO, year_value)
        analyze_ltc_feature(KU_ALL_XIN_ALL_AUTO, year_value)

if __name__ == "__main__":
    #anlyze_sk_rank()    
    #analyze_sk_rank_start_with_different_models()
