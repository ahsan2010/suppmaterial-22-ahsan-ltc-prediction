...
# -*- coding: utf-8 -*-
"""
Created on Mon Feb 19 23:38:10 2024
...
def analyze_sk_rank_ku_xin_combined(year_list, model_type_list):
    result_list = []
    for year in year_list:
        print(year)
        for model_type in model_type_list:
            location = "{}sk_model_input_year_{}{}_ku_xin_combined.txt".format(sk_analysis_dir,year, model_type)
            df = analyze_ltc_feature(location)
            df['ltc'] = year
            df['model_type'] = 'no-auto' if model_type=='' else 'auto'
            result_list.append(df)
    pd_final = pd.concat(result_list)
    pd_final.to_csv('{}sk_rank_results_ku_xin_combined_analysis.csv'.format(sk_rank_result_dir),index = False)
...
def main():
    ...
    analyze_sk_rank_ku_xin_combined(year_list , model_type_list)
